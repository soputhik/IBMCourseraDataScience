...
app.layout = html.Div(children=[html.H1('SpaceX Launch Records Dashboard',
    ...
    dcc.RangeSlider(
                                    id='payload-slider', min=0, max=10000, step=1000, value=[0, 10000]),
...
def get_scatter_chart(entered_site, payload):
    ...
    if (entered_site == 'ALL'):
        fig = px.scatter(filtered_df, x='Payload Mass (kg)', y='class',
                         color='Booster Version Category', title='Payload Mass Success Count on Launch Sites')
        return fig
    else:
        fig = px.scatter(filtered_df[filtered_df['Launch Site'] == entered_site], x='Payload Mass (kg)',
                         y='class', color='Booster Version Category', title=f'Total Successful Launches on {entered_site}')
        return fig
